...
# output file
BIN_NAME=$(echo "$1" | cut -b 7-)
OUTFILE=$(echo "./results/$BIN_NAME-results.csv")

echo "Trace, IPC,Total Access,Total Hits,Total Misses,Total MPKI,Load Access,Load Hit,Load Miss,Load MPKI,Data Load MPKI,Prefetch Requested,Prefetch Issued,Prefetch Useful,Prefetch Useless,Useful Load Prefetches,Timely Prefetches,Late Prefetches,Dropped Prefetches,Average Miss Latency,Accuracy,L1 MPKI, LLC MPKI" > "$OUTFILE"

for TRACE in "${TRACES[@]}"; do
    echo "Running $TRACE ..."

    # Run ChampSim and capture output
    OUTPUT=$("$1" -warmup_instructions 25000000 -simulation_instructions 25000000 -traces ./traces/$TRACE)
    ...
    L1D_TOTAL_MPKI=$(echo "$OUTPUT" | grep "L1D TOTAL     ACCESS" | awk '{print $16}')
    LLC_TOTAL_MPKI=$(echo "$OUTPUT" | grep "LLC TOTAL     ACCESS" | awk '{print $16}')
    ...
    ACCURACY=$(echo "$OUTPUT" | grep "L2C USEFUL LOAD PREFETCHES" | awk '{print $13}')
    
    # Build row
    echo "$TRACE,$IPC,$L2_TOTAL_ACCESS,$L2_TOTAL_HIT,$L2_TOTAL_MISS,$L2_TOTAL_MPKI,$L2_LOAD_ACCESS,$L2_LOAD_HIT,$L2_LOAD_MISS,$L2_LOAD_MPKI,$L2_DATA_LOAD_MPKI,$L2C_PREFETCH_REQUESTED,$L2C_PREFETCH_ISSUED,$L2C_PREFETCH_USEFUL,$L2C_PREFETCH_USELESS,$L2C_PREFETCH_USEFUL_LOAD_PREFETCHES,$L2C_TIMELY_PREFETCHES,$L2C_LATE_PREFETCHES,$L2C_DROPPED_PREFETCHES,$L2C_AVERAGE_MISS_LATENCY,$ACCURACY,$L1D_TOTAL_MPKI,$LLC_TOTAL_MPKI" >> "$OUTFILE"
done
